...
impl EmployeeManagementContract {
    pub fn initialize(
        ...
        env.storage().instance().set(&DataKey::Institution, &institution_info);

        let empty_list: Vec<Address> = Vec::new(&env);
        env.storage().persistent().set(&DataKey::EmployeeList, &empty_list);
    ...
    pub fn reactivate_employee(
        ...
        match employee_data.status {
            EmployeeStatus::Active => {
                return Err(EmployeeError::EmployeeAlreadyActive);
            }
            EmployeeStatus::Suspended => {
    ...
    pub fn get_employee(env: Env, employee: Address) -> Result<Employee, EmployeeError> {
        ...
        env.storage().persistent().get(&employee_key).ok_or(EmployeeError::EmployeeNotFound)
    }
    ...
    pub fn get_institution_info(env: Env) -> Result<InstitutionInfo, EmployeeError> {
        ...
        env.storage().instance().get(&DataKey::Institution).ok_or(EmployeeError::StorageError)
    }

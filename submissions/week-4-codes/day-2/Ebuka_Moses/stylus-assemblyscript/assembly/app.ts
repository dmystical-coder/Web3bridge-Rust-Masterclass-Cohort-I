...
function averagePrice(prices:i32[]):i32{
  let sum = 0;
...
function getAveragePriceFromIntegers(prices:StaticArray<i32>):i32{
  if (prices.length ===0){
    return 0;
  }
  let sum:i64 = 0;
...
export const main = (input:i32[]):i32=>{
  const average = averagePrice(input);
  return average;
}

...
impl ERC6909 {
    ...
    pub fn _mint(&mut self, to: Address, id: U256, amount: U256) -> Result<(), ERC6909Error> {
        if to.is_zero() {
            return Err(
                ERC6909Error::ERC6909InvalidReciever(ERC6909InvalidReciever { reciever: to })
            );
        }
    ...
    pub fn _update(
        ...
        if !from.is_zero() {
            ...
            if fromBalance < amount {
                return Err(
                    ERC6909Error::ERC6909InsufficientBalance(ERC6909InsufficientBalance {
                        sender: from,
                        balance: fromBalance,
                        needed: amount,
                        id,
    ...
    pub fn _approve(
        ...
        if owner.is_zero() {
            return Err(
                ERC6909Error::ERC6909InvalidApprover(ERC6909InvalidApprover { approver: owner })
            );
        }
        if spender.is_zero() {
            return Err(
                ERC6909Error::ERC6909InvalidSpender(ERC6909InvalidSpender {
                    spender,
                })
            );
        }

        self._allowances.setter(owner).setter(spender).setter(id).set(amount);
        Ok(())
    }
    ...
    pub fn _setOperator(
        ...
        self._operatorApprovals.setter(owner).setter(spender).set(approved);
        Ok(())
    }
    ...
    pub fn _spendAllowance(
        ...
        if allowance < amount {
            return Err(
                ERC6909Error::ERC6909InsufficientAllowance(ERC6909InsufficientAllowance {
                    needed: spender,
                    id: allowance,
                })
            );
